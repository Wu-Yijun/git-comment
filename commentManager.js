...
const DefaultConsts = {
  ...
  user:
      ` <img class="Github-Login-User-Icon" src="/GithubIcon.svg" height="25px">
...
class GitControl {
  ...
  constructor(oauthInfo) {
    ...
    if (this.access_token &&
        (typeof this.access_token === 'string' ||
         this.access_token instanceof String) &&
  ...
  enableLogin(reset = false) {
    Array.prototype.forEach.call(
        document.getElementsByClassName(this.oauthInfo.GitLoginClass),
        (dom) => {
          if (reset) dom.innerHTML = DefaultConsts.log;
          setTimeout(() => {
            dom.onclick = () => {
  ...
  userInfo = {
    /*icon*/
    avatar_url:
        'https://avatars.githubusercontent.com/u/126391865?v=4',  // 1234
    // name
    login: 'Wu-Yijun',
    // github id
    id: 126391865,
  ...
  CreateComment(content, callback) {
    ...
    const url = `https://api.github.com/repos/${oauthInfo.Owner}/${
        oauthInfo.Repo}/issues/${oauthInfo.Issue}/comments`;
    const header = {
...
export default class commentManager {
  constructor(config) {
    ...
    this.domContainer = config.dom ||
        document.getElementsByClassName('comment-contents')[0] ||
  ...
  addJson(j) {
    ...
    try {
      ...
      console.log(
          'Processing body of #' + json.id + ' interrupted with error:\n', e);
    }
  ...
  getDom(index) {
    if (index < 0) index = 0;
    if (index >= this.numberPerPage) index = this.numberPerPage - 1;
    return this.doms[index];
  }
  setDomComment(index, id) {
    const comment = this.getComment(id);
    const dom = this.getDom(index);
    if (comment.valid && (!comment.deleted)) {
    ...
    if (comment.quote.exist) {
      ...
      if (comment.quote.refered) {
        ...
        setHTML(
            dom, 'comment-quote-date',
            new Date(comment.quote.date).toLocaleString());
        setACTION(dom, 'comment-quote-toolbar', (el) => {
    ...
    setACTION(dom, 'comment-entry', (el) => {
      if (el.clientHeight < el.scrollHeight) overflowed = true;
    });
    if (overflowed) {
  ...
  bindTextarea() {
    Array.prototype.forEach.call(
        document.getElementsByClassName('comment-textarea'), (dom) => {
